package org.apache.spark.shuffle.sort.collection
...
import org.apache.spark.shuffle.gpft.Revertable
...
/**
  * Mostly adapted from Spark's ExternalSorter implementation.
  * 1. Spark's ExternalSorter couples with a single Task, this restrict ExternalSorter's lifespan inside a single task
  * 2. Will not merge the spills
  * 3. Implement revertable interface that support commit and revert
  */
 ...
 */
private[spark] class SharedExternalSorter[K, V, C](
    shuffleId: Int,
    _dep: ShuffleDependency[K, V, C],
    taskMemoryManager: TaskMemoryManager,
    partitioner: Option[Partitioner] = None,
    ordering: Option[Ordering[K]] = None,
    serializer: Serializer = SparkEnv.get.serializer)
  extends Spillable[WritablePartitionedPairCollection[K, C]](taskMemoryManager) with Revertable
  with Logging {

  /** mapper id of current owner task */
  private var mapId: Int = -1

  /** task context of current owner task */
  private var context: TaskContext = _

  /** aggregator of current owner task (aggregator should not be shared among tasks because it has shared SerializerInstance) */
  private var aggregator: Option[Aggregator[K, V, C]] = None

  /**
    * True if there has not been any spill since the beginning of current owner task. The first spill of a task is
    * considered its shuffle output.
    */
  private var taskFirstSpill: Boolean = false

  /** shuffleBlockResolver of current owner task */
  private var shuffleBlockResolver: IndexShuffleBlockResolver = _

  /** mapStatus will be updated upon the first spill */
  private var mapStatus: Option[MapStatus] = _

  private var blockManager = SparkEnv.get.blockManager
  private var diskBlockManager = blockManager.diskBlockManager
  private var serializerManager = SparkEnv.get.serializerManager
  private var serInstance = serializer.newInstance()

  /** Every time a new task to insert elements into this sorter, ownership must be reset to update some per-thread states */
  def resetOwnership(mapId: Int, context: TaskContext, aggregator: Option[Aggregator[K, V, C]], shuffleBlockResolver: IndexShuffleBlockResolver): Unit = {
    this.mapId = mapId
    this.context = context
    this.aggregator = aggregator
    this.taskFirstSpill = true
    this.shuffleBlockResolver = shuffleBlockResolver
    this.mapStatus = None

    this.blockManager = SparkEnv.get.blockManager
    this.diskBlockManager = blockManager.diskBlockManager
    this.serializerManager = SparkEnv.get.serializerManager
  ...
  /**
    * @param records
    * @return true if there is map output file generated, otherwise false
    */
  def insertAll(records: Iterator[Product2[K, V]]): MapStatus = {
    // TODO: stop combining if we find that the reduction factor isn't high
    val shouldCombine = aggregator.isDefined

    if (shouldCombine) {
      // Combine values in-memory first using our AppendOnlyMap
      logInfo(s"stage ${context.stageId()} task ${context.partitionId()} use ExternalSorter with map-side combine")
  ...
  /**
    * Because Spark requires to use partitioned containers that has a (pid, key) as map key. What we want is just to
    * forcefully set the partition id to 0, rather than partition id. This is useful if we want to aggregate with
    * executor-side combining technique, and then distributed to different aggregators at next level in treeAggregate.
    * @param records
    * @return true if there is map output file generated, otherwise false
    */
  def insertAllFixedPartitionId(records: Iterator[Product2[K, V]]): MapStatus = {
    val shouldCombine = aggregator.isDefined
    require(shouldCombine, "insertAllForceKeys is only meaningful when mapSideCombine is on")

    // Combine values in-memory first using our AppendOnlyMap
    logInfo(s"stage ${context.stageId()} task ${context.partitionId()} use ExternalSorter with map-side combine and fixed partition id")
    val mergeValue = aggregator.get.mergeValue
    val createCombiner = aggregator.get.createCombiner
    var kv: Product2[K, V] = null
    val update = (hadValue: Boolean, oldValue: C) => {
    ...
    while (records.hasNext) {
      addElementsRead()
      kv = records.next()
      map.changeValue((0, kv._1), update)
      maybeSpillCollection(usingMap = true)
    }
    if (mapStatus.isDefined) {
      mapStatus.get
  ...
  /** Flush and stop the sorter */
  def flush(): MapStatus = {
    logInfo(s"stage ${context.stageId()} flush task ${mapId} flushed")
    val output = shuffleBlockResolver.getDataFile(shuffleId, mapId)
    val tmp = org.apache.spark.util.Utils.tempFileWith(output)
    try {
      val blockId = ShuffleBlockId(shuffleId, mapId, IndexShuffleBlockResolver.NOOP_REDUCE_ID)
      val partitionLengths = writePartitionedFile(blockId, tmp)
      shuffleBlockResolver.writeIndexFileAndCommit(shuffleId, mapId, partitionLengths, tmp)
      mapStatus = Some(MapStatus(blockManager.shuffleServerId, partitionLengths))
    } finally {
      if (tmp.exists() && !tmp.delete()) {
        logError(s"Error while deleting temp file ${tmp.getAbsolutePath}")
      }
    }
    logInfo(s"stage ${context.stageId()} flush task ${mapId} write its map output file into ${tmp.getAbsolutePath}")
    mapStatus.get
  }
  ...
  /**
    * Modification:
    * 1. We treat the first spill as map output
    * 2. Remaining is treated the same as spill
    */
  /**
  ...
  override protected[this] def spill(collection: WritablePartitionedPairCollection[K, C]): Unit = {
    if (taskFirstSpill) {
      ...
      logInfo(s"stage ${context.stageId()} partition ${context.partitionId()} write its map output file into ${tmp.getAbsolutePath}")
    } else {
      val inMemoryIterator = collection.destructiveSortedWritablePartitionedIterator(comparator)
      val spillFile = spillMemoryIteratorToDisk(inMemoryIterator)
      spills += spillFile
      logInfo(s"stage ${context.stageId()} partition ${context.partitionId()} successfully written to a new spill file ${spillFile.file.getAbsolutePath} (${spills.length})")
    }
  }
  ...
  override def commit(): Unit = {
    throw new NotImplementedError("Fault tolerance not implemented")
  }
